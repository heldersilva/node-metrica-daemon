...
module.exports = function (server){
  ...
  server.get('/os/mem', function(request, response, next){
    async.parallel({
      ...
      "percentage-free": function(done){
        done(null, (os.freemem() / os.totalmem()).toFixed(2));
      },
      "percentage-use": function(done){
        done(null, ((os.totalmem() - os.freemem()) / os.totalmem()).toFixed(2));
      }
    },
    function (err, result){
      next.ifError(err);
